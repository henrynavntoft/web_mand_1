...
async function mix_fetch_data(el) {
  ...
  if (el.getAttribute("mix-wait")) {
    ...
    document
      .querySelector(el.getAttribute("mix-wait"))
      .classList.remove("mix-hidden");
  }
  let conn = null;
...
function process_template(mix_url) {
  ...
  if (document.querySelector("template[mix-redirect]")) {
    ...
    location.href = document
      .querySelector("template[mix-redirect]")
      .getAttribute("mix-redirect");
    return;
  }
  ...
  document.querySelectorAll("template[mix-target]").forEach((template) => {
    ...
    console.log(
      `ok : mix() the response data will affect '${template.getAttribute(
        "mix-target"
      )}'`
    );
    ...
    if (position == "innerHTML") {
      document.querySelector(template.getAttribute("mix-target")).innerHTML =
        template.innerHTML;
    } else if (position == "replace") {
      document
        .querySelector(template.getAttribute("mix-target"))
        .insertAdjacentHTML("afterend", template.innerHTML);
      document.querySelector(template.getAttribute("mix-target")).remove();
    } else {
      document
        .querySelector(template.getAttribute("mix-target"))
        .insertAdjacentHTML(position, template.innerHTML);
    }
...
function mixonurl(mix_url, push_to_history = true) {
  ...
  document.querySelectorAll(`[mix-on-url='${mix_url}']`).forEach((el) => {
    ...
    if (el.getAttribute("mix-push-url") && push_to_history) {
      ...
      history.pushState(
        { mixonurl: el.getAttribute("mix-push-url") },
        "",
        el.getAttribute("mix-push-url")
      );
...
function mix_convert() {
  // cl("converting")
  document
    .querySelectorAll("[mix-get], [mix-delete], [mix-put], [mix-post]")
    .forEach((el) => {
      // cl(el)
      let method = "mix-get";
      if (el.hasAttribute("mix-delete")) {
